...
def set_up_logger(args, sys_argv):
    # set up running log
    n_walks, n_steps = args.n_walks, args.n_steps
    runtime_id = '{}-{}-{}-{}-{}-{}-{}'.format(str(time.time()), args.data, args.mode[0], args.agg, n_steps, n_walks, args.pos_dim)
...
def save_oneline_result(dir, args, test_results):
    ...
    with open(dir+'oneline_results.txt', 'a') as f:
        elements = [str(e) for e in [args.data, args.mode[0], args.agg, n_steps, n_walks, args.pos_enc, args.pos_dim, args.walk_pool, *[str(v)[:6] for v in test_results]]]
